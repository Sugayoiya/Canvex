...
must_haves:
  truths:
    ...
    - "Celery tasks call ProviderManager + ImageGenerationService/VideoService with proper async bridge and asyncio.wait_for timeout (110s image, 280s video)"
...
IMAGE_PROVIDER_TIMEOUT = 110  # Leave buffer below Celery task soft limit
VIDEO_PROVIDER_TIMEOUT = 280  # Leave buffer below Celery task soft limit
...
async def _async_generate_image(prompt, aspect_ratio, model, team_id, user_id):
    ...
    try:
        # Wrap with asyncio.wait_for to prevent indefinite hang (addresses review concern #2)
...
```
  ...
  <acceptance_criteria>
    ...
    - `_async_generate_image` wraps provider call with `asyncio.wait_for(timeout=IMAGE_PROVIDER_TIMEOUT)` where `IMAGE_PROVIDER_TIMEOUT = 110`
    - `_async_generate_video` wraps provider call with `asyncio.wait_for(timeout=VIDEO_PROVIDER_TIMEOUT)` where `VIDEO_PROVIDER_TIMEOUT = 280`
    - Both async helpers catch `asyncio.TimeoutError` separately and report to `KeyHealthManager`
...
3. Provider calls wrapped with asyncio.wait_for() (110s image, 280s video) to prevent indefinite hang
4. Tasks create/update SkillExecutionLog per D-21
