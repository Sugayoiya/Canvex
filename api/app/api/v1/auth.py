...
# ---------------------------------------------------------------------------
# Rate limiter (imported from main.py shared instance)
# ---------------------------------------------------------------------------
try:
...
def _apply_limit(limit_string: str):
    """Decorator factory: apply rate limit if limiter is available."""
    def decorator(func):
...
# ---------------------------------------------------------------------------
# OAuth state store (in-memory fallback; Redis preferred in production)
# ---------------------------------------------------------------------------
_consumed_oauth_states: set[str] = set()
_MAX_STATE_CACHE = 1000


def _consume_oauth_state(state: str) -> bool:
    """Verify and consume an OAuth state token (single-use)."""
    try:
        payload = jwt.decode(state, settings.SECRET_KEY, algorithms=[ALGORITHM])
    except Exception:
        return False
    nonce = payload.get("nonce")
    if not nonce or nonce in _consumed_oauth_states:
        return False
    _consumed_oauth_states.add(nonce)
    if len(_consumed_oauth_states) > _MAX_STATE_CACHE:
        evict = list(_consumed_oauth_states)[:len(_consumed_oauth_states) // 2]
        _consumed_oauth_states.difference_update(evict)
    return True
...
# ---------------------------------------------------------------------------
# OAuth exchange endpoint (frontend calls this with the short-lived code)
# ---------------------------------------------------------------------------

# NOTE: in-process dict; works for single-worker. Multi-worker deployments
# should replace with Redis SETEX to avoid random exchange failures.
_pending_oauth_codes: dict[str, tuple[str, str, float]] = {}


def _store_oauth_code(access_token: str, refresh_token: str) -> str:
    """Create a short-lived one-time code that can be exchanged for tokens."""
    code = secrets.token_urlsafe(48)
    _pending_oauth_codes[code] = (
        access_token,
        refresh_token,
        datetime.now(timezone.utc).timestamp() + 60,
    )
    if len(_pending_oauth_codes) > 500:
        now = datetime.now(timezone.utc).timestamp()
        expired = [k for k, v in _pending_oauth_codes.items() if v[2] < now]
        for k in expired:
            _pending_oauth_codes.pop(k, None)
    return code


@router.post("/oauth/exchange")
async def exchange_oauth_code(body: dict):
    """Exchange a short-lived OAuth code for access + refresh tokens."""
    code = body.get("code", "")
...
async def google_callback(code: str, state: str, db: AsyncSession = Depends(get_db)):
    if not _consume_oauth_state(state):
        raise HTTPException(status_code=400, detail="Invalid or reused OAuth state")
    ...
    oauth_code = _store_oauth_code(access_token, refresh_token)
    redirect_url = f"{settings.FRONTEND_URL}/login?oauth_code={oauth_code}"
